...
typedef struct {
    size_t N, kc_bytes;
    const int8_t* A; size_t a_row_stride, a_stride_bytes;
    const int8_t* B;
    int8_t* C; size_t c_row_stride, cm_stride_bytes, cn_stride_bytes;
    requantization_params_t requant_params;
} int8_qgemm_ctx_t;

#define INT8_QGEMM_CHUNK_BODY(UK7, UK1)                                         \
    int8_qgemm_ctx_t* c = (int8_qgemm_ctx_t*)ctx;                               \
    ...
    while (row < row_end) {                                                     \
        ...
        if (rows_left >= 7) {                                                   \
            UK7(7, c->N, c->kc_bytes,                                           \
                ...
                c->cn_stride_bytes, c->requant_params);                         \
            row += 7;                                                           \
        } else {                                                                \
...
static void int8_qgemm_chunk(size_t row_begin, size_t row_end, void* ctx) {
    INT8_QGEMM_CHUNK_BODY(qgemm_i8_i32_7xm4, qgemm_i8_i32_1xm4)
}
static void int8_qgemm_relu_chunk(size_t row_begin, size_t row_end, void* ctx) {
    INT8_QGEMM_CHUNK_BODY(qgemm_i8_i32_7xm4_relu, qgemm_i8_i32_1xm4_relu)
}
#undef INT8_QGEMM_CHUNK_BODY
...
void int8_qgemm(
    ...
    requantization_params_t requant_params)
{
    int8_qgemm_ctx_t ctx = {
        .N = N, .kc_bytes = K,
        .A = A, .a_row_stride = a_row_stride, .a_stride_bytes = a_row_stride,
        .B = B,
        .C = C, .c_row_stride = c_row_stride, .cm_stride_bytes = c_row_stride,
        .cn_stride_bytes = c_col_stride,
        .requant_params = requant_params,
    };
    nn_rvv_parallel_for(M, int8_qgemm_chunk, &ctx);
}
    ...
    requantization_params_t requant_params)
{
    ...
    nn_rvv_parallel_for(M, int8_qgemm_relu_chunk, &ctx);
}
