...
void conv_1x1_relu_f32(
    ...
    for (size_t ch_o = 0; ch_o < channels_out; ch_o++) {
        ...
        for (size_t ch_i = 0; ch_i < channels_in; ch_i++) {
            ...
            weight = *(weights++);
            if (channels_in == 1){
                conv_1x1_relu_f32_2d(rows, cols, input_ptr, weight, bias_value, output);
            } else if (ch_i == 0) {
                conv_1x1_f32_2d(rows, cols, input_ptr, weight, bias_value, output);
            } else if (ch_i != (channels_in - 1)) {
                conv_1x1_f32_2d_macc(rows, cols, input_ptr, weight, output);
            } else {
                conv_1x1_relu_f32_2d_macc(rows, cols, input_ptr, weight, output);
            }
        }
        output += rows * cols;
    }
